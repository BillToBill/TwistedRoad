...
static const int		W_TEXT_X = 884;
static const int		W_TEXT_Y = 778;
...
void BugginOutTextGenerator::appendTimerText(Game *game)
{
	wstringstream wss;
	WindowsInput *input = (WindowsInput*)game->getInput();
	int seconds = game->getTimer()->getTotalTime();
	wss << seconds;
	textToGenerate.append(wss.str());
}
void BugginOutTextGenerator::appendLivesText(Game *game)
{
	wstringstream wss;
	SpriteManager *manager = game->getGSM()->getSpriteManager();
	wss << L"Lives: ";
	wss << manager->getPlayerLives();
	wss << L"\n";
	textToGenerate.append(wss.str());
}

void BugginOutTextGenerator::appendHealthText(Game *game)
{
	SpriteManager *manager = game->getGSM()->getSpriteManager();
	wstringstream wss;
	wss << L"Health: ";
	wss << manager->getPlayerHP();
	wss << L"\n";
	textToGenerate.append(wss.str());
}
