...
{
	getSpriteManager()->clearSprites();
	b2Body* player = getSpriteManager()->getPlayer()->body;
	list<AnimatedSprite*>::iterator blocks = getSpriteManager()->getBlocksIterator();
	list<AnimatedSprite*>::iterator worldExits = getSpriteManager()->getExitsIterator();
	list<AnimatedSprite*>::iterator enemies = getSpriteManager()->getEnemyIterator();

	b2World* gameWorld = getPhysics()->getBoxWorld();

	gameWorld->DestroyBody(player);

	for (blocks; blocks != getSpriteManager()->getEndOfBlocksIterator(); blocks++)
	{
		AnimatedSprite* block = *blocks;
		b2Body* toBeDestroyed = block->body;
		gameWorld->DestroyBody(toBeDestroyed);
	}

	for (enemies; enemies != getSpriteManager()->getEndOfEnemyIterator(); enemies++)
	{
		AnimatedSprite* enemy = *enemies;
		b2Body* toBeDestroyed = enemy->body;
		gameWorld->DestroyBody(toBeDestroyed);
	}

	for (worldExits; worldExits != getSpriteManager()->getEndOfExitsIterator(); worldExits++)
	{
		AnimatedSprite* worldExit = *worldExits;
		b2Body* toBeDestroyed = worldExit->body;
		gameWorld->DestroyBody(toBeDestroyed);
	}

	list<b2Body*>::iterator bodyIt;
	bodyIt = getSpriteManager()->getStaticBodiesIterator();
	for (bodyIt; bodyIt != getSpriteManager()->getEndOfStaticBodiesIterator(); bodyIt++)
	{
		b2Body* bod = (*bodyIt);
		gameWorld->DestroyBody(bod);
	}

	getSpriteManager()->clearStaticBodies();
