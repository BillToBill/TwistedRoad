...
void Game::quitGame()
{
	...
	list<AnimatedSprite*>::iterator blocksIterator;
	blocksIterator = gsm->getSpriteManager()->getBlocksIterator();
	b2Body* playerBody = gsm->getSpriteManager()->getPlayer()->body;

	/*while (blocksIterator != gsm->getSpriteManager()->getEndOfBlocksIterator())
	{
		world->DestroyBody((*blocksIterator)->body);
		blocksIterator++;
	}*/
